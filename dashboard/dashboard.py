...
# Importando os textos e mensagens
from texts import (
...
class YouTubeDashboard:
    ...
    def show_channel_analysis(self):
        st.title(TITLE_CHANNEL_ANALYSIS)
        st.markdown(DESC_CHANNEL_ANALYSIS)
    ...
    def display_additional_info(self):
        ...
        with col2:
            st.markdown(f"**{METRIC_FIRST_VIDEO_CHANNEL}:** {canal_primeiro_video}")
 ...
 '**Resultado**: Não há evidência estatística suficiente para afirmar que o dia da semana afeta significativamente as visualizações (p > 0.05).'}
    ...
    def save_suggestion_to_db(self, channel_sugestion, dashboard):
        ...
        try:
            ...
            session = Session()
            suggestion = {"channel_name": channel_sugestion, "dashboard": dashboard}
